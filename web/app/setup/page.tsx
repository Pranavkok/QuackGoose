...
export default function SetupPage() {
  ...
  // If user already has an org, skip to admin dashboard
  useEffect(() => {
    fetch('/api/org/me')
      .then(r => r.json())
      .then((d: { orgId?: string }) => {
        if (d.orgId) router.replace('/org/dashboard');
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.push('/org/dashboard?welcome=1');
    } catch (e) {
      ...
      <div className="mx-auto max-w-lg">
          ...
          {[1, 2].map((s, i) => (
            <div key={s} className="flex items-center">
              <div className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold transition-all ${
                ...
                : step === s ? 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-200'
                : 'border-2 border-gray-200 text-gray-400'
              }`}>
                {step > s ? '✓' : s}
              </div>
              {i < 1 && <div className={`mx-1 h-0.5 w-16 rounded-full transition-all ${step > s ? 'bg-green-400' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
          {step === 1 && (
            <div className="p-8">
              <div className="mb-8">
                <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-3xl shadow-lg shadow-blue-200">🏢</div>
                <h1 className="text-2xl font-black text-gray-900">Set up your organization</h1>
                <p className="mt-1.5 text-gray-500">Create your team&apos;s workspace to start managing productivity.</p>
              </div>
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="flex overflow-hidden rounded-xl border border-gray-200 transition-all focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/15">
                    <span className="shrink-0 border-r border-gray-200 bg-gray-50 px-3 py-3 text-sm text-gray-400">quackfocus.com/</span>
                    <input
                      type="text" value={slug}
                      onChange={e => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 32))}
                      placeholder="acme-technologies"
                      className="flex-1 bg-white px-3 py-3 text-sm text-gray-900 outline-none"
                    />
                  </div>
                  {slug && <p className="mt-1 text-xs text-gray-400">Your URL: <span className="font-medium text-gray-600">quackfocus.com/{slug}</span></p>}
                </div>
              </div>
              <button
          ...
          {step === 2 && (
            <div className="p-8">
              <div className="mb-8">
                <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-purple-700 text-3xl shadow-lg shadow-violet-200">📋</div>
                <h1 className="text-2xl font-black text-gray-900">Tell us about your team</h1>
                <p className="mt-1.5 text-gray-500">We&apos;ll personalize your setup for the right plan.</p>
              </div>
                    ...
                    {INDUSTRIES.map(ind => (
                      <button key={ind} type="button" onClick={() => setIndustry(ind)}
                        className={`rounded-xl border px-3 py-2.5 text-left text-sm font-medium transition-all ${
                          industry === ind ? 'border-blue-600 bg-blue-50 text-blue-700 ring-1 ring-blue-600'
                    ...
                    {TEAM_SIZES.map(s => (
                      <button key={s} type="button" onClick={() => setTeamSize(s)}
                        className={`rounded-xl border px-5 py-2.5 text-sm font-semibold transition-all ${
                          teamSize === s ? 'border-blue-600 bg-blue-50 text-blue-700 ring-1 ring-blue-600'
              ...
              {error && <div className="mt-5 rounded-xl bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</div>}
              <div className="mt-8 flex gap-3">
                <button onClick={() => setStep(1)} className="flex-1 rounded-xl border border-gray-200 py-3.5 text-sm font-semibold text-gray-600 transition-colors hover:bg-gray-50">← Back</button>
                <button
                ...
                >
                  {loading ? (
                    <><svg className="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Creating workspace…</>
                  ) : 'Launch workspace 🚀'}
                </button>
              </div>
            </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-400">14-day free trial · No credit card required · Cancel anytime</p>
      </div>
    </div>
  );
}
